...
## 6. Hosting Constraints & HostPapa Status
...
4. **Current Build Output Gap (Outcome C)**:
   - Build emits Nitro server bundle `.output/server/index.mjs` and client assets in `.output/public/assets/`, but **no static `index.html`** in `.output/public/`.
   - Direct upload to `public_html/` is not supported by the current build.
5. **Phase 1 Deliverable**: Transition to a prebuilt static artifact (`index.html` + asset bundles) deployable via local build → Git push → HostPapa Git pull.
