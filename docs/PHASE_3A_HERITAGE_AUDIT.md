...
| **Archive Gallery** (`/gallery`) | 24 mock items filterable by 4 categories and 3 eras; full-screen lightbox modal with metadata and previous/next controls. | **Modal focus leak & drop**: Opening the lightbox does NOT move focus inside the dialog (focus remains on opening card button); tabbing navigates background page cards behind the modal. Closing from inside the modal drops focus to `document.body` rather than restoring to the trigger. | Conflicts with expected modal-dialog/WAI-ARIA Authoring Practices behavior and raises WCAG focus-order/conformance concerns (final formal WCAG mapping reserved for Phase 8 certification). Touch targets on 320px wrap cleanly without horizontal overflow. Escape key and backdrop click dismiss modal. | **FIX REGARDLESS**: Migrate lightbox to standard dialog component (e.g. Radix Dialog) to provide focus containment, initial focus management, and automatic focus restoration upon close. |
...
All heritage and memorial test journeys were evaluated using automated browser execution against the live application server (`PORT 4182`) across Desktop (1280×900), Tablet (768×1024), Mobile (390×844), and Small Mobile (320×568) in both English (`en`, LTR) and Arabic (`ar`, RTL).

Structured assertion records are permanently tracked in [`docs/evidence/phase-3a/20260918-phase-3a-heritage/browser-test-records.json`](evidence/phase-3a/20260918-phase-3a-heritage/browser-test-records.json).
...
| :--- | :--- | :--- | :--- |
| `journey1_airport.index_en` | `/airport` (1280×900) | Airport Overview Chapters | 3 chapter cards rendered (`cardCount: 3`) linking to Past, Present, and Future chapters with honest archival provenance disclaimer. |
| `journey1_airport.timeline_analysis` | `/airport/past` (1280×900) | Historical Milestones Timeline | 5 chronological milestone entries inspected. Confirms 1998 opening milestone carries mock Verified badge while 4 other milestones carry explicit Placeholder pills with replacement notices. |
| `journey1_airport.present_facts` | `/airport/present` (1280×900) | Factual Definition List | Confirms 4 key factual data points in `<dl>`: Site (Southern Gaza Strip, near Rafah), IATA code (`GZA`), Opened (`1998`), and Current status (`Not operating — pending documentation`). |
| `journey2_gallery` | `/gallery` (1280×900) | Gallery Filters & Empty States | Confirmed category filtering (`architectureFilterCount: 6`), dual filter intersection (`comboFilterCount: 2`), circular keyboard navigation (`keyboardNextTitle`), and empty state reset button structure. |
| `journey3_destinations.amm_detail` | `/destinations/AMM` (1280×900) | Destination Detail Route Collision | Confirmed critical route occlusion defect: `renderedH1: "Destinations"`, `hasSearchForm: false`, `hasDestSearchInput: true`, `containsWeeklySchedule: false`. Parent route `{-$locale}.destinations.tsx` lacks `<Outlet />`, occluding child component `DestinationPage`. |
| `journey4_visitor.contactSentVerification` | `/contact` (1280×900) | Contact Form Submission Mutation | Confirmed mock message submission updates UI state to confirmation panel with explicit unrouted disclaimer (*"Your message has not been sent anywhere yet — messaging is not connected."*) and reset button (`hasResetButton: true`). |
| `keyboardA11y.galleryLightboxEscapeCloses` | `/gallery` (1280×900) | Gallery Lightbox Keyboard Dismissal | Confirmed pressing `Escape` key cleanly closes the lightbox modal (`galleryLightboxEscapeCloses: true`). |
| `modalBehavior.galleryLightbox` | `/gallery` (1280×900) | Lightbox Modal Semantics | Confirmed modal markup: `hasRoleDialog: true`, `ariaModal: "true"`, `ariaLabel: "Item viewer"`, with Close, Prev, and Next buttons present. |
| `responsiveAudits.gallery320` | `/gallery` (320×568) | Narrow Mobile Responsiveness | Confirmed 320px viewport layout resilience: `bodyScrollWidth: 320`, `windowInnerWidth: 320`, `hasHorizontalOverflow: false` with wrapping filter pills. |
| `deepA11yInspections.galleryLightbox` | `/gallery` (1280×900) | Lightbox Focus Trapping & Restoration | Evaluated dialog focus order: initial focus remains on trigger card, tabbing leaks to underlying page cards behind overlay, and closing after interacting inside modal drops focus to `document.body` (focus-management behavior conflicting with expected modal-dialog/WAI-ARIA behavior and raising WCAG focus-order concerns). |
| `deepA11yInspections.travelNavigation` | `/travel` (1280×900) | Travel Section Navigation | Confirmed 5 section buttons in `<nav aria-label="Travel information">` using `aria-current="true"`. Lacks WAI-ARIA `tablist`/`tab` roles; relies on Tab order rather than Arrow key navigation. |
| `deepA11yInspections.timelineSemantics` | `/airport/past` (1280×900) | Milestone Timeline Heading & List Semantics | Confirmed semantic `<ol>` structure with 5 milestone sections, H1 followed by H2 headings, and figure/figcaption elements. |
...
- **Homepage Entry**: Scrolled to the dark ink section, three full-bleed cards invite exploration with kicker "The airport", title "One airport, told in three chapters", and subtitle "Its history, its present state, and the vision for its future." Each card carries an Arabic/English label and forward arrow.
- **Landing Hub (`/airport`)**: Displays `PageHeader` with title *"Gaza International Airport"* and subtitle *"One airport, told in three chapters."* Three prominent cards present chapters 01 (Past), 02 (Present), and 03 (Future) with aspect-ratio 16:10 photography, numbering (`numeral`), and read buttons (`airport.readChapter`).
...
- **Sources & Metadata Callout**: At the foot of the chapter, a callout box clarifies:
...
- **Visual Shift**: Intentionally transitions from dark-ink memory to a neutral stone/sand light surface (`bg-secondary`), signaling documentary factual reporting rather than evocative retrospection.
...
- **Architectural Concept Themes**: Three thematic articles with alternating image/copy alignment:
...
- **Item Inventory**: Initial render presents 24 items, filterable by 4 categories (*Photographs*, *Documents*, *Architecture*, *Concepts*) and 3 eras (*Past*, *Present*, *Future*).
...
- **Filtering Mutations**:
  - Selecting "Architecture" mutates the item count from 24 to 6.
  - Adding "Future" era filter mutates the item count to 2 (`architectureFilterCount: 6`, `comboFilterCount: 2`).
...
- **Interaction**: Clicking any gallery card opens the full-screen lightbox modal.
...
- **Navigation Controls**: Previous (`ChevronLeft`) and Next (`ChevronRight`) buttons cycle items circularly. Keyboard shortcuts `ArrowRight` and `ArrowLeft` smoothly transition items in place.
...
- **Measured Keyboard & Focus Behaviors**:
  ...
  - *Focus drop upon close (from inside dialog)*: If the user focuses a control inside the modal (e.g. Close, Prev, or Next buttons) and dismisses via Escape or backdrop click, `src/routes/{-$locale}.gallery.tsx` does not restore focus to the opening card button; focus drops to `document.body` (`tagName: 'BODY'`, `isBody: true`), which conflicts with expected modal-dialog/WAI-ARIA Authoring Practices behavior and raises WCAG focus-order concerns.
...
- **Directory**: Displays 7 regional destinations in the opening network: Amman (`AMM`), Cairo (`CAI`), Istanbul (`IST`), Doha (`DOH`), Dubai (`DXB`), Jeddah (`JED`), and Riyadh (`RUH`).
...
- **Search Filtering**:
  - Typing `"cairo"` instantly filters to a single card: Cairo International (`CAI`, Egypt, 1h 15m, from $139).
  - Typing an unserved city like `"london"` displays a clean empty state: *"Nothing matches these filters yet."*
...
- **Section Navigation**: Desktop layout features a sticky left sidebar (`w-64`) with `<nav aria-label="Travel information">` containing an unordered list (`<ul>`) of 5 `<button>` elements corresponding to topical sections:
  ...
  3. *Baggage* (`baggage`): 7 kg cabin bag (55 × 40 × 20 cm), 23 kg checked bag, $35 extra bag fee.
  4. *At the airport* (`airport`): Check-in, security, passport control, quiet areas and family rooms.
  5. *Accessibility* (`accessibility`): Kerb-to-gate wheelchair assistance, step-free paths, assistance animals.
...
- **Mobile Responsive Layout**: On 390px mobile, the vertical sidebar switches to a horizontally scrolling navigation bar (`overflow-x-auto pb-2`), preserving screen real estate while keeping all 5 sections accessible.
...
- **Form Layout**: Clean 2-column form card with Name, Email, Subject dropdown (Booking enquiry, Baggage, Accessibility, Archive contribution, Media), and Message textarea.
- **HTML5 Validation**: Clicking "Send message" with empty fields triggers native browser validation tooltips on the first required input (`#c-name`), preventing blank submissions.
- **Prototype Submission State**: Submitting valid data transitions local state to a confirmation panel:
...
- **Institutional Context**: Features two balanced panels:
...
1. **Missing `<Outlet />` in `src/routes/{-$locale}.destinations.tsx`**:
   ...
   - **Evidence**: Browser test record `journey3_destinations.amm_detail` and source inspection of `src/routes/{-$locale}.destinations.tsx`.
...
2. **Missing `/airport/present` in Site Footer**:
   ...
   - **Evidence**: Source inspection of `src/components/site-footer.tsx` lines 17–24 and browser evaluation of footer links.
...
3. **Homepage Travel Quick Links Lack Section Anchoring**:
   ...
   - **Evidence**: Source inspection of `src/routes/{-$locale}.index.tsx` lines 195–204 and browser evaluation of homepage quick links.
...
1. **Gallery Lightbox Focus Leak and Focus Drop to Body on Close**:
   - **Type**: Accessibility Defect (conflicts with modal-dialog/WAI-ARIA Authoring Practices behavior and raises WCAG focus-order concerns; final formal WCAG mapping reserved for Phase 8 certification).
   - **Evidence**: Retained browser test record `deepA11yInspections.galleryLightbox`.
...
3. **Airport Trilogy Lacks Unifying Sub-Navigation Bar**:
   ...
   - **Evidence**: Route inspection across `/airport/past`, `/airport/present`, and `/airport/future`.
...
1. **Exemplary Provenance & Transparency Disclaimers**:
   ...
   - **Evidence**: Retained browser test records `journey1_airport.timeline_analysis` and source inspection of `src/lib/data.ts`.
...
2. **Honest Prototype Form Feedback**:
   ...
   - **Evidence**: Retained browser test record `journey4_visitor.contactSentVerification`.
...
3. **Filter Pill Wrap & Touch Targets on 320px**:
   ...
   - **Evidence**: Retained browser test record `responsiveAudits.gallery320`.
...
- **Evidence**: Retained browser test records `journey1_airport.index_en` and `journey1_airport.timeline_analysis`.
- **Genuine Strengths**:
...
- **Evidence**: Retained browser test record `journey1_airport.timeline_analysis` and `src/lib/data.ts` lines 513–578.
- **Genuine Strengths**:
...
- **Evidence**: Route inspection of `/airport/past`, `/airport/present`, and `/airport/future`.
- **Genuine Strengths**:
...
- **Evidence**: Retained browser test records `journey2_gallery` and `deepA11yInspections.galleryLightbox`.
- **Genuine Strengths**:
  - Fast client-side filtering by category and era.
  - Clean image presentation with credit and ID attributes.
- **Defects vs Opportunities**:
  - *Confirmed Defect*: Modal focus leak — opening the lightbox leaves focus on the opening card button; tabbing cycles through background buttons behind the modal overlay (conflicting with expected modal-dialog/WAI-ARIA Authoring Practices behavior and raising WCAG focus-order concerns).
  - *Confirmed Defect*: Modal focus drop on close — when focus is moved inside the dialog (e.g. to Close or Next buttons), closing via Escape or backdrop click drops focus to `document.body` rather than restoring to the opening card button (conflicting with expected modal-dialog/WAI-ARIA Authoring Practices behavior and raising WCAG focus-order concerns).
- **Materially Distinct Options**:
...
- **Impacts**:
  - A11y: Resolves modal focus containment, focus-order concerns, and dialog accessibility semantics for the gallery lightbox (aligning with expected modal-dialog/WAI-ARIA Authoring Practices behavior; final formal WCAG mapping reserved for Phase 8 certification).
...
- **Evidence**: Retained browser test record `journey3_destinations.amm_detail` and source inspection of `src/routes/{-$locale}.destinations.tsx`.
- **Genuine Strengths**:
...
- **Evidence**: Retained browser test record `deepA11yInspections.travelNavigation` and source inspection of `src/routes/{-$locale}.travel.tsx`.
- **Genuine Strengths**:
